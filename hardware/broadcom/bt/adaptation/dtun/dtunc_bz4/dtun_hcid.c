...
//--- BRCM_LOCAL
	...
	for(i = 0; i < len; i++)
	{
//+++ BRCM_LOCAL : [CASE#437592] Fix ASCII validation error(ASCII Control character or extended ASCII). Without the fix remote device name will be over-written to just dots only when RNR completed.[CASE#437592] Fix ASCII validation error(ASCII Control character or extended ASCII). Without the fix remote device name will be over-written to just dots only when RNR completed.
#if 0
...
void dtun_dm_sig_authorize_req(tDTUN_DEVICE_SIGNAL *msg)
{
	...
	struct btd_device *device;
//+++ BRCM LOCAL : euc-kr device name check
...
//--- BRCM LOCAL
	...
	/* Don't use cod from BTA if it's invalid. Fix for CASE ID 317920 */
	if(msg->authorize_req.info.cod != 0)
	{
		info( "cod changed update to %x", msg->authorize_req.info.cod );
...
//+++ BRCM LOCAL : euc-kr device name check
		...
		tmpName[DTUN_MAX_DEV_NAME_LEN-1] = '\0';
