...
case TRANSACTION_getCurrentSync:
{
data.enforceInterface(DESCRIPTOR);
android.content.SyncInfo _result = this.getCurrentSync();
reply.writeNoException();
if ((_result!=null)) {
reply.writeInt(1);
...
public android.content.SyncInfo getCurrentSync() throws android.os.RemoteException
{
android.os.Parcel _data = android.os.Parcel.obtain();
android.os.Parcel _reply = android.os.Parcel.obtain();
android.content.SyncInfo _result;
try {
_data.writeInterfaceToken(DESCRIPTOR);
mRemote.transact(Stub.TRANSACTION_getCurrentSync, _data, _reply, 0);
_reply.readException();
if ((0!=_reply.readInt())) {
_result = android.content.SyncInfo.CREATOR.createFromParcel(_reply);
}
else {
_result = null;
}
...
static final int TRANSACTION_getCurrentSync = (android.os.IBinder.FIRST_CALL_TRANSACTION + 15);
